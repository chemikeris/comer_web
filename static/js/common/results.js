...
function showResults(results) {
    ...
    if (multimer_search) {
        ...
        for (var i = 0; i < query_chains.length; i++) {
            ...
            var w = Math.floor(100000 * (chain_info.length / results[results_header].query.length)) / 1000;
    ...
    for (var i = 0; i < number_of_hits; i++) {
        ...
        if (multimer_search) {
            ...
            for (var j = 0; j < hit_record.assignment_table.length; j++) {
                ...
                current_chain_summary_div.appendChild(createLinkToAlignment(`${i}_${chain_correspondence_info.refrn_chain}`, chain_correspondence_info.refrn_chain));
        ...
        if (multimer_search) {
            ...
            for (var j = 0; j < hit_record.alignment_section.length; j++) {
                ...
                var alignment_for_one_chain_div = formatAlignment(i, hr, structure_search, multimer_search);
...
function formatAlignment(result_no, hit_record, structure_search, multimer_search) {
    ...
    if (multimer_search) {
        header.id = 'alignment_' + result_no + '_' + hit_record.refrn_chain;
    }
    else {
