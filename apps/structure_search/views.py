...
def results(request, job_id):
    ...
    if finished and not removed:
        ...
        context = {
            ...
            'job_input': '' if removed else input_url_a,
            'sequences': [],
            'active': 'not_finished',
            }
...
def detailed(request, job_id, result_no):
    ...
    results[job.first_results_header()]['sort_order'] = \
        models.parse_gtalign_job_options(options_str)['--sort']
    processed_results = models.prepare_results_json(
        results, job.first_results_header()
        )
    query_desc = utils.format_gtalign_description(
        results[job.first_results_header()]['query']['description']
        )
